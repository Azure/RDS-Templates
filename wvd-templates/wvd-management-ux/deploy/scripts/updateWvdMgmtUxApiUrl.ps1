...
if ($RoleAssignment.RoleDefinitionName -eq "Owner" -or $RoleAssignment.RoleDefinitionName -eq "Contributor")
{
	...
	$ListWebApp = Get-AzWebApp | Where-Object { $_.Name -eq $webAppName }
	...
	$ApiUrl = "https://$webappName-api.azurewebsites.net"
	...
	foreach ($Publishedfile in $AllPublishedFiles) {
		if ($Publishedfile.href -like "*/main*") {
			...
			$ListkuduApiUrl = "https://$webAppName.scm.azurewebsites.net/api/vfs/site/wwwroot/$MainBundleJsonFileName"
			...
			$kuduApiUrlUpdate = "https://$webAppName.scm.azurewebsites.net/api/vfs/site/wwwroot/$MainBundleJsonFileName"
