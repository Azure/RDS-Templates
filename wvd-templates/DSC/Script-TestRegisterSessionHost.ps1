...
if (!$SessionHost) {
    Write-Log -Message "SessionHost '$SessionHostName' does not exist in Host Pool '$HostPoolName' in Tenant '$TenantName'"
    return $false
}
if ($SessionHost.Status -ne 'Available') {
    Write-Log -Message "SessionHost '$SessionHostName' is not available. $($SessionHost | Out-String)"
    return $false
}
