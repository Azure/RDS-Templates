...
try {
	...
	foreach ($SessionHost in $SessionHosts) {
		...
		if (Get-PSObjectPropVal -Obj $SessionHost -Key 'AzureResourceId') {
			$VMsLookup.Add($SessionHost.AzureResourceId, $VM)
		}
		elseif (Get-PSObjectPropVal -Obj $SessionHost -Key 'AzureVmId') {
			$VMsLookup.Add($SessionHost.AzureVmId, $VM)
		}
	}
